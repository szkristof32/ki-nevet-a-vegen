...
{
	/** Attach callback. Called when layer is pushed onto a layer stack */
	void (*on_attach)();
	/** Detach callback. Called when layer is popped from a layer stack */
	void (*on_detach)();
	/** Update callback. Called when the game loop processes the layer */
	void (*on_update)(float timestep);
	/** UI render callback. Called when the game UI is rendererd */
	void (*on_ui_render)();
	/** Window resize callback */
	void (*on_window_resize)(uint32_t width, uint32_t height);
	/** Pointer to a block of memory owned by each layer */
	void* internal_state;
} layer_t;
...
/**
 * Pushes a layer from the stack
 *
 * @param layer_stack A pointer to a `layer_stack_t` struct to be popped from
 * @param layer A pointer to a `layer_t` struct to be popped from the stack
 */
